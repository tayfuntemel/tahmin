...
# Loglama ayarları (İşlemleri konsolda rahatça takip edebilmen için)
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# Veritabanı bağlantı ayarları (GitHub Secrets üzerinden gelecek)
DB_CONFIG = {
...
def ensure_table():
    """model_calibration tablosunu yoksa oluşturur."""
    conn = None
    cursor = None
    try:
        conn = get_connection()
...
def calculate_bias(days_back=30, min_matches=20):
    """Geçmiş maçların sonucuna göre Beklenen Gol (xG) sapmasını hesaplar."""
    ...
    try:
        ...
        if row and row['total_matches'] >= min_matches:
            ...
            logging.warning(f"Kalibrasyon için yeterli veri yok (İhtiyaç: {min_matches}, Mevcut: {row['total_matches'] if row else 0})")
            return None, None
    finally:
        cursor.close()
        conn.close()
...
def update_calibration_params(home_bias, away_bias):
    """Hesaplanan sapmaları veritabanına yazar."""
    ...
    try:
        ...
        # Değerleri Güncelle (Yoksa ekle, varsa üzerine yaz)
        cursor.execute("""
...
def main():
    ...
    try:
        ...
        logging.error(f"Betiğin çalışması sırasında kritik hata: {e}")
