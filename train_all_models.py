...
train_all_models.py
-------------------
- results_football tablosundaki bitmiş maçları, belirtilen maksimum yıl ve haftaya kadar kullanarak
...
# ---------- 2. ANALİTİK TABLOLARI DOLDUR (sadece <= max_year,max_week olan maçlarla) ----------
def populate_analytics_tables(max_year, max_week):
    """
    results_football'daki bitmiş maçları kullanarak analitik tabloları günceller.
    Sadece (match_year < max_year) veya (match_year = max_year AND match_week <= max_week) olan maçlar kullanılır.
    """
    conn = get_connection()
    print(f"[ANALİTİK] Tablolar dolduruluyor (yıl<={max_year}, hafta<={max_week})... Bu biraz sürebilir.")

    # Filtre koşulu
    date_filter = f"""
        WHERE status IN ('finished','ended')
          AND match_year IS NOT NULL AND match_week IS NOT NULL
          AND (match_year < {max_year} OR (match_year = {max_year} AND match_week <= {max_week}))
    """
    ...
    # 2. team_form_analytics (son 5 maç puanı)
    query_form = f"""
    ...
    query_referee = f"""
        ...
        SELECT 
            ...
            AVG(ft_home + ft_away) as avg_goals
        FROM results_football
        {date_filter.replace("AND match_year IS NOT NULL AND match_week IS NOT NULL", "")}
        GROUP BY referee
...
# ---------- 3. EĞİTİM VERİSİNİ YÜKLE (sadece <= max_year,max_week) ----------
def load_training_data(max_year, max_week):
    conn = get_connection()
    date_filter = f"""
        AND r.match_year IS NOT NULL AND r.match_week IS NOT NULL
        AND (r.match_year < {max_year} OR (r.match_year = {max_year} AND r.match_week <= {max_week}))
    """
    query = f"""
    SELECT 
        r.event_id,
    ...
    conn.close()
    print(f"{len(df)} bitmiş maç yüklendi (yıl<={max_year}, hafta<={max_week}).")
    return df
...
def main():
    ...
    if args.max_year is not None and args.max_week is not None:
        ...
        print(f"===== TÜM MODELLERİ EĞİTME SİSTEMİ (sınır: {max_year} hafta {max_week}'e kadar) =====")
    else:
        # Parametre verilmemişse: tüm veriyi kullan, son yıl/haftayı bul
        conn = get_connection()
        cursor = conn.cursor()
        ...
        max_year, max_week = last_year, last_week
        print(f"===== TÜM MODELLERİ EĞİTME SİSTEMİ (tüm veriler, son hafta: {max_year} hafta {max_week}) =====")

    # 1. Tabloları oluştur
    create_analytics_tables()

    # 2. Tabloları doldur (sadece filtrelenmiş maçlarla)
    populate_analytics_tables(max_year, max_week)

    # 3. Eğitim verisini yükle (yine aynı filtreyle)
    df_raw = load_training_data(max_year, max_week)
    if len(df_raw) < 10:
        print("Yetersiz veri (10'dan az maç). Eğitim yapılamıyor.")
        sys.exit(0)
    ...
    # 6. Versiyon etiketi
    version_suffix = f"{max_year}_week{max_week}"
