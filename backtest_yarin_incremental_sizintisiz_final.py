...
# -*- coding: utf-8 -*-
"""
SIZINTISIZ YARIN TAHMIN MOTORU

Mantık:
- Tahmin tarihi varsayılan olarak Türkiye saatine göre yarındır.
- İstatistik cutoff tarihi = tahmin tarihi - 1 gün.
- Cache boşsa veya FORCE_FULL_REBUILD=1 ise cutoff tarihine kadar full cache kurar.
- Cache doluysa varsayılan olarak sadece cutoff gününde oynanan maçların lig/takım/hakem istatistiklerini günceller.
- Sonra sadece tahmin tarihindeki maçlara prediction kaydı yazar.
- H2H, lig, takım formu, hakem ve takım tahmin başarı oranı tahmin gününü veya sonrasını görmez.

Kullanım:
    python3 backtest_yarin_incremental_sizintisiz_final.py
    python3 backtest_yarin_incremental_sizintisiz_final.py 2026-05-05
    BACKTEST_PREDICT_DATE=2026-05-05 python3 backtest_yarin_incremental_sizintisiz_final.py
...
# =========================
# YARIN TAHMIN AYARLARI
# =========================
TR_TZ = ZoneInfo("Europe/Istanbul")
DEFAULT_PREDICT_DATE = dt.datetime.now(TR_TZ).date() + dt.timedelta(days=1)

# Tarihi koddan/env/komut satırından kontrol edebilirsin.
# Kullanım örnekleri:
#   python3 backtest_yarin_incremental_sizintisiz_final.py
#   python3 backtest_yarin_incremental_sizintisiz_final.py 2026-05-05
#   BACKTEST_PREDICT_DATE=2026-05-05 python3 backtest_yarin_incremental_sizintisiz_final.py
#
# Komut satırındaki tarih varsa BACKTEST_PREDICT_DATE üzerine yazar.
...
_cli_predict_date = sys.argv[1] if len(sys.argv) >= 2 and sys.argv[1].strip() else None
PREDICT_DATE = parse_date(_cli_predict_date or os.getenv("BACKTEST_PREDICT_DATE"), DEFAULT_PREDICT_DATE)
CUTOFF_DATE = PREDICT_DATE - dt.timedelta(days=1)

# Aynı tahmin tarihindeki eski prediction kayıtlarını silip baştan yazsın mı?
CLEAR_EXISTING_PREDICTIONS = os.getenv("CLEAR_EXISTING_PREDICTIONS", "1").strip().lower() not in {"0", "false", "no", "hayir", "hayır"}

# Cache güncelleme modu:
#   auto        -> cache boşsa full, doluysa incremental
#   full        -> cutoff tarihine kadar tüm cache tablolarını baştan kurar
#   incremental -> sadece cutoff gününde maç yapan lig/takım/hakemleri günceller
STATS_MODE = os.getenv("STATS_MODE", "auto").strip().lower()
FORCE_FULL_REBUILD = os.getenv("FORCE_FULL_REBUILD", "0").strip().lower() in {"1", "true", "yes", "evet"}
...
def clear_predictions_for_day(db: BacktestDB, predict_date: dt.date):
    db.cursor.execute("DELETE FROM predictions WHERE prediction_date = %s", (predict_date,))
    print(f"🧹 Eski tahminler temizlendi: prediction_date={predict_date}")


def cache_has_enough_data(db: BacktestDB) -> bool:
    try:
        db.cursor.execute("SELECT COUNT(*) AS c FROM league_stats")
        league_count = int((db.cursor.fetchone() or {}).get('c') or 0)
        db.cursor.execute("SELECT COUNT(*) AS c FROM team_form_cache")
        team_count = int((db.cursor.fetchone() or {}).get('c') or 0)
        return league_count > 0 and team_count > 0
    except Exception:
        return False
...
def main():
    ...
    try:
        print("\n" + "=" * 72)
        print(f"YARIN SIZINTISIZ TAHMIN | cutoff={CUTOFF_DATE} | predict_date={PREDICT_DATE}")
        print(f"STATS_MODE={STATS_MODE} | FORCE_FULL_REBUILD={int(FORCE_FULL_REBUILD)} | CLEAR_EXISTING_PREDICTIONS={int(CLEAR_EXISTING_PREDICTIONS)}")

        if CLEAR_EXISTING_PREDICTIONS:
            clear_predictions_for_day(db, PREDICT_DATE)

        updater = BacktestStatsUpdater(db, CUTOFF_DATE)
        has_cache = cache_has_enough_data(db)
        if FORCE_FULL_REBUILD or STATS_MODE == "full" or (STATS_MODE == "auto" and not has_cache):
            updater.update_full()
        elif STATS_MODE in {"auto", "incremental"}:
            updater.update_incremental(changed_date=CUTOFF_DATE)
        else:
            raise ValueError("STATS_MODE sadece auto, full veya incremental olabilir")

        saved = BacktestPredictionEngine(db, PREDICT_DATE).run_for_day()

        print("\n" + "=" * 72)
        print(f"✅ Yarın tahmini tamamlandı. Tahmin tarihi: {PREDICT_DATE} | cutoff: {CUTOFF_DATE} | kaydedilen tahmin: {saved}")
        print("Not: Cache son hali cutoff tarihine göre kalır. Bu sızıntısız çalışma için normaldir.")
    finally:
        db.close()
