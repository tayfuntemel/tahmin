...
# -*- coding: utf-8 -*-
"""
SIZINTISIZ 90 GUNLUK BACKTEST MOTORU

Mantık:
- START_PREDICT_DATE = 2026-02-04 ise ilk istatistik cutoff'u 2026-02-03'tür.
...
Kullanım:
    python3 backtest_90gun_incremental_sizintisiz_final.py 2026-05-04
...
# BACKTEST AYARLARI
# =========================
DEFAULT_START_PREDICT_DATE = dt.date(2026, 2, 4)
DEFAULT_BACKTEST_DAYS = 90  # yaklaşık 90 gün

# Tarihleri koddan/env/komut satırından kontrol edebilirsin.
# Kullanım örnekleri:
#   python3 backtest_90gun_incremental_sizintisiz_final.py
#   python3 backtest_90gun_incremental_sizintisiz_final.py 2026-05-04
#   BACKTEST_START_DATE=2026-02-04 BACKTEST_END_DATE=2026-05-04 python3 backtest_90gun_incremental_sizintisiz_final.py
#
# Not: Komut satırında tek tarih verirsen bu END_PREDICT_DATE olarak yorumlanır.
...
START_PREDICT_DATE = parse_date(os.getenv("BACKTEST_START_DATE"), DEFAULT_START_PREDICT_DATE)
_cli_end_date = sys.argv[1] if len(sys.argv) >= 2 and sys.argv[1].strip() else None
END_PREDICT_DATE = parse_date(_cli_end_date or os.getenv("BACKTEST_END_DATE"), START_PREDICT_DATE + dt.timedelta(days=DEFAULT_BACKTEST_DAYS - 1))
if END_PREDICT_DATE < START_PREDICT_DATE:
    raise ValueError(f"END_PREDICT_DATE START_PREDICT_DATE'den küçük olamaz: {END_PREDICT_DATE} < {START_PREDICT_DATE}")
BACKTEST_DAYS = (END_PREDICT_DATE - START_PREDICT_DATE).days + 1

# Aynı tarih aralığında önceden üretilmiş tahminler varsa temizleyip baştan yazsın mı?
# Varsayılan True. Resume yapmak istersen env ile kapatabilirsin:
#   CLEAR_EXISTING_BACKTEST_PREDICTIONS=0 python3 backtest_90gun_incremental_sizintisiz_final.py 2026-05-04
CLEAR_EXISTING_BACKTEST_PREDICTIONS = os.getenv("CLEAR_EXISTING_BACKTEST_PREDICTIONS", "1").strip().lower() not in {"0", "false", "no", "hayir", "hayır"}
