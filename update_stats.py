...
class StatsUpdater:
    ...
    def update_league_stats(self):
        """Lig bazlı ortalamaları hesapla ve kaydet"""
        query = """
            SELECT 
                category_id,
    ...
    def update_team_dna(self):
        """Her takım için tüm geçmiş ortalamalar"""
        query = """
            SELECT 
        ...
        self.cursor.execute(query)
        rows = self.cursor.fetchall()
        # Birleştir ve ortalama al (ev/deplasman aynı takım için)
        team_stats = defaultdict(lambda: {'goals': [], 'shot_on': [], 'corners': [], 'count': 0})
        for row in rows:
            key = f"{row['team_name']}|{row['category_id']}"
            team_stats[key]['goals'].append(row['avg_goals'])
            team_stats[key]['shot_on'].append(row['avg_shot_on'])
            team_stats[key]['corners'].append(row['avg_corners'])
            team_stats[key]['count'] += row['total_matches']
        
        today = dt.date.today()
        for key, stats in team_stats.items():
            avg_goals = sum(stats['goals']) / len(stats['goals'])
            avg_shot_on = sum(stats['shot_on']) / len(stats['shot_on'])
            avg_corners = sum(stats['corners']) / len(stats['corners'])
            insert_sql = """
    ...
    def update_team_form_cache(self):
        """Her takım için son 10 maçlık form ve son 3 maçlık hücum verileri"""
        # Önce tüm benzersiz takım_key'leri al
        ...
        for team_key in team_keys:
            ...
            avg_shot_on_10 = sum(shot_on) / len(shot_on) if shot_on else 0
            
            # Korner ortalaması
            corners = []
            for m in matches:
                if m['home_team'] == team_name:
                    corners.append(m['corn_h'] or 0)
                else:
                    corners.append(m['corn_a'] or 0)
            avg_corners_10 = sum(corners) / len(corners) if corners else 0
            
            # BTTS oranı
            btts_count = 0
            for m in matches:
                home_g = m['ft_home'] or 0
                away_g = m['ft_away'] or 0
                if home_g > 0 and away_g > 0:
                    btts_count += 1
            btts_ratio = btts_count / len(matches)
            
            # İç saha son 5 maç gol ortalaması
            self.cursor.execute("""
            ...
            home_goals = [m['ft_home'] or 0 for m in home_matches]
            home_avg = sum(home_goals) / len(home_goals) if home_goals else 0
            
            # Dış saha son 5 maç gol ortalaması
            self.cursor.execute("""
            ...
            away_goals = [m['ft_away'] or 0 for m in away_matches]
            ...
            insert_sql = """
                ...
                (team_key, as_of_date, last_10_avg_goals, last_10_avg_shot_on, last_10_avg_corners,
                 last_10_btts_ratio, home_last_5_avg_goals, away_last_5_avg_goals,
                 last_3_avg_shot_on, last_3_avg_corners, last_3_avg_possession)
            ...
            self.cursor.execute(insert_sql, (
                team_key, today,
                avg_goals_10, avg_shot_on_10, avg_corners_10,
                btts_ratio, home_avg, away_avg,
                avg_shot_on_3, avg_corners_3, avg_poss_3
            ))
    ...
    def update_referee_stats(self):
        ...
        for row in rows:
            insert_sql = """
                ...
                last_updated = VALUES(last_updated)
            """
            self.cursor.execute(insert_sql, (row['referee'], row['avg_goals'], row['match_count'], today))
