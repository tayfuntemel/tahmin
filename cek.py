...
class Scraper:
    ...
    def _fetch_json(self, target_url: str) -> dict:
        try:
            ...
            print(f"[İSTEK] {url_son[:40]}... -> ScraperAPI")
            response = requests.get('http://api.scraperapi.com', params=payload, timeout=60)
            if response.status_code == 200:
                data = response.json()
                if "error" in data:
                    print(f"[HATA] {url_son} -> {data['error']}")
                    return {}
                return data
            else:
...
def main():
    ...
    # Dün, bugün ve yarın – fikstür için yeterli
    ...
    for date_str in target_dates_str:
        ...
        for ev in events:
            ...
            if status not in ["notstarted", "scheduled"]:
                continue
            ev_id = ev.get("id")
            odds_data = sc.get_odds(ev_id)
            row = sc.parse(ev, odds_data)
            db.upsert_match(row)
            count_for_date += 1
            total_processed += 1
        if count_for_date > 0:
            print(f"[{date_str}] {count_for_date} maç (odds+fikstür) güncellendi.")

    db.close()
    print(f"Toplam {total_processed} maç işlendi. Script tamamlandı.")
    if total_processed == 0:
        print("[UYARI] Hiç maç bulunamadı veya işlenemedi.")
        sys.exit(1)
