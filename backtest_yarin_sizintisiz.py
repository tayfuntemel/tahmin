...
Mantık:
- Hedef tarih otomatik olarak Europe/Istanbul saatine göre yarındır.
- Önce sadece bugün dahil bitmiş maçlarla cache tabloları yeniden kurulur.
- Sonra sadece yarının maçlarına tahmin oluşturulur.
- prediction_date CURDATE() değil, scriptin hesapladığı hedef tarihtir.
- H2H, lig, takım formu, hakem ve takım tahmin başarı oranı hedef günü veya sonrasını görmez.

Kullanım:
    python3 backtest_yarin_sizintisiz.py

İstersen hedef tarihi elle override edebilirsin:
    BACKTEST_PREDICT_DATE=2026-05-05 python3 backtest_yarin_sizintisiz.py
"""
...
def get_target_predict_date() -> dt.date:
    manual_date = os.getenv("BACKTEST_PREDICT_DATE", "").strip()
    if manual_date:
        return dt.date.fromisoformat(manual_date)
    return dt.datetime.now(ISTANBUL_TZ).date() + dt.timedelta(days=1)

START_PREDICT_DATE = get_target_predict_date()
BACKTEST_DAYS = 1  # sadece yarının tahmini

# Yarın için önceden tahmin üretildiyse temizleyip aynı mantıkla baştan yazar.
# Sadece prediction_date = yarın olan kayıtları etkiler.
...
class BacktestStatsUpdater:
    ...
    def update_all(self):
        print(f"\n📊 İSTATİSTİK CACHE -> cutoff: {self.cutoff_date}")
    ...
    def clear_cache_tables(self):
        # Backtestte gelecekten kalmış cache değeri sızmasın diye her gün sıfırdan kurulur.
        # Dikkat: Bu tablolar canlı cache tablolarıdır; script bitince son günün cutoff cache'i kalır.
    ...
    def update_league_stats(self):
        ...
        print(f"  league_stats: {len(rows)} lig")
    ...
    def update_team_form_cache(self):
        ...
        for row in teams:
            ...
            count += 1
        print(f"  team_form_cache: {count} takım")
    ...
    def update_team_dna(self):
        ...
        print(f"  team_dna: {len(rows)} takım")
    ...
    def update_referee_stats(self):
        ...
        print(f"  referee_stats: {len(rows)} hakem")
...
def main():
    ...
    try:
        ...
        for i in range(BACKTEST_DAYS):
            ...
            BacktestStatsUpdater(db, cutoff_date).update_all()
