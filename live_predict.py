#!/usr/bin/env python3
"""
live_predict.py - Bugün ve yarınki maçlar için tahmin yapar.
İstatistikler dün ve öncesindeki tüm bitmiş maçlarla hesaplanır,
modeller bu veriyle eğitilir ve gelecek maçlar tahmin edilir.
"""
...
# Hiperparametre optimizasyonu isteniyorsa True, yoksa False
USE_HYPEROPT = True

# ==========================================
# 2. TABLO ŞEMALARI (analiz tabloları ve tahmin tablosu)
# ==========================================
SCHEMAS = {
...
class Database:
    ...
    def truncate_analytics_tables(self):
        """İstatistik tablolarını sıfırlar (canlıda her çalıştırmada yeniden oluşturmak için)."""
        tables = [
...
# ==========================================
# 4. ANALİZ SINIFLARI (Efficiency, TeamGeneral, HalfTime, SecondHalf, Form, League, Referee)
# ==========================================
class EfficiencyAnalyzer:
    ...
    def analyze(self, matches):
        ...
        for key, data in self.stats.items():
            ...
            count += 1
        print(f"[VERİMLİLİK] {count} takım güncellendi.")
...
class TeamGeneralAnalyzer:
    ...
    def analyze(self, matches):
        ...
        for key, data in self.stats.items():
            ...
            self.db.cur.execute(insert_query, row)
            count += 1
        print(f"[GENEL] {count} kayıt güncellendi.")
...
class HalfTimeAnalyzer:
    ...
    def analyze(self, matches):
        ...
        for key, data in self.stats.items():
            ...
            self.db.cur.execute(insert_query, row)
            count += 1
        print(f"[İLK YARI] {count} takım güncellendi.")
...
class SecondHalfAnalyzer:
    ...
    def analyze(self, matches):
        ...
        for key, data in self.stats.items():
            ...
            self.db.cur.execute(insert_query, row)
            count += 1
        print(f"[İKİNCİ YARI] {count} takım güncellendi.")
...
class FormAnalyzer:
    ...
    def analyze(self, matches):
        ...
        for key, data in self.stats.items():
            ...
            self.db.cur.execute(insert_query, row)
            count += 1
        print(f"[FORM] {count} takım güncellendi.")
...
class LeagueAnalyzer:
    ...
    def analyze(self, matches):
        ...
        for t_id, lg in self.leagues.items():
            ...
            self.db.cur.execute(insert_query, row)
            count += 1
        print(f"[LİG] {count} lig güncellendi.")
...
class RefereeAnalyzer:
    ...
    def analyze(self, matches):
        ...
        for ref_name, r in self.referees.items():
            ...
            self.db.cur.execute(insert_query, row)
            count += 1
        print(f"[HAKEM] {count} hakem güncellendi.")

def run_all_analyzers(db, max_date=None):
    """Tüm istatistik tablolarını verilen tarihe kadar yeniden oluşturur."""
    db.truncate_analytics_tables()
    matches = db.get_matches_finished(max_date)
    if not matches:
        print("   Hiç bitmiş maç bulunamadı.")
        return
    analyzers = [
...
def prepare_features(df):
    ...
    after = len(X)
    if before - after > 0:
        print(f"    Eğitimde {before - after} satır eksik veri nedeniyle silindi.")
...
# ==========================================
# 6. CANLI TAHMİN (BUGÜN VE YARIN)
# ==========================================
def predict_upcoming_matches(db):
    try:
    ...
    end_date = today + timedelta(days=2)  # bugün ve yarın

    query = get_ml_features_query() + f"""
        WHERE r.start_utc >= '{today}' AND r.start_utc <= '{end_date}'
    ...
    df = pd.DataFrame(db.cur.fetchall())
    if df.empty:
        print("    Bugün veya yarın için maç bulunamadı.")
        return
    ...
    for idx, row in df.iterrows():
        ...
        if missing:
            ...
            print(f"    Eksik veri -> NO_BET: {row['home_team']} - {row['away_team']}")
        else:
            ...
            if market != 'NO_BET':
                predicted_count += 1
                print(f"    {row['home_team']} - {row['away_team']} -> {market} (%{prob*100:.1f})")
            else:
                print(f"    {row['home_team']} - {row['away_team']} -> NO_BET (max=%{max(prob_o15,prob_o25,prob_o35)*100:.1f})")
        ...
        """, (row['event_id'], market, prob, prob_o15, prob_o25, prob_o35))

    print(f"\n>>> Toplam {predicted_count} maça tahmin üretildi.")
...
def main():
    print("=" * 60)
    print(" CANLI TAHMIN OLUŞTURMA (BUGÜN & YARIN)")
    print("=" * 60)
    ...
    # Bugünün tarihini al (gece yarısından itibaren, gelecek maçları eğitime dahil etme)
    tz_tr = timezone(timedelta(hours=3))
    today_date = datetime.now(tz_tr).date()

    print(f"\n1. İstatistikler hesaplanıyor (tarih < {today_date})...")
    run_all_analyzers(db, max_date=today_date)

    print("\n2. Modeller eğitiliyor...")
    success = train_models(db, max_date=today_date)
    if not success:
        print("Model eğitilemedi, çıkılıyor.")
        db.close()
        sys.exit(1)

    print("\n3. Bugün ve yarınki maçlar için tahmin yapılıyor...")
    predict_upcoming_matches(db)
