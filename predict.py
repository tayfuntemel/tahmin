...
MAJOR_TOURNAMENT_IDS = { ... }  # aynı (kısaltıldı)
...
class PredictionEngine:
    ...
    def get_team_form_cache(self, team_key, as_of_date):
        self.cursor.execute("""
            SELECT * FROM team_form_cache 
            WHERE team_key = %s AND as_of_date <= %s 
    ...
    def calculate_form_score(self, home_cache, away_cache, league_avg_goals):
        ...
        norm_general = min(general_avg / league_avg_goals, 1.5)
        general_score = min(31.5, general_avg * 7 * norm_general)
        # Özel form (iç saha son 5 vs dış saha son 5)
        specific_avg = (home_cache['home_last_5_avg_goals'] + away_cache['away_last_5_avg_goals']) / 2.0
        norm_specific = min(specific_avg / league_avg_goals, 1.5)
    ...
    def calculate_attack_pressure(self, home_cache, away_cache, league_avg_shot_on, league_avg_corners):
        def pressure(cache):
            ...
            # Kesici kural: son maç isabetli şut <=1 ise poss katkısı 0
            # Bu bilgi cache'de yok, varsayılan olarak poss katkısını kullan
            return (
    ...
    def calculate_early_bonus(self, home_cache, away_cache, league_avg_goals):
        ...
        # Takımların ilk yarı gol ortalaması = toplam golün %40'ı varsay
    ...
    def predict_match(self, match):
        ...
        home_cache = self.get_team_form_cache(home_key, today)
        away_cache = self.get_team_form_cache(away_key, today)
    ...
    def run(self):
        ...
        for match in matches:
            ...
            print(f"✅ {pred['home_team']} vs {pred['away_team']} -> Over Edge: {pred['over_edge']:.1f}% / KG Edge: {pred['btts_edge']:.1f}%")
        self.close()
